...
const resolveActiveItem = (pathname: string, items: NavbarItem[]): string | null => {
...
const syncNavbarState = () => {
  ...
  for (const navbar of navbars) {
    ...
    if (toggleIcon) {
      const isOpen = (navbar as HTMLElement & { mobileMenuOpen?: boolean }).mobileMenuOpen === true;
      toggleIcon.name = isOpen ? 'x' : 'menu';

      if (!navbar.dataset.toggleIconBound) {
        const onMenuChange = (event: Event | string) => {
